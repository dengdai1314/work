...
public class CameraActivity extends BaseActivity implements View.OnClickListener {
    ...
    int camera_flag = 0;//wifi按钮按下后，点击+/-才可以进行跳转活动操作
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.key_wifi:
                camera_flag = camera_flag+1;//wifi按钮按下，flag+1,然后音量+/-键才可以进行操作
                goScan();
                break;
            case R.id.key_up:
                if(camera_flag!=0){
    ...
    private void goScan(){
        ...
        //跳转回调扫描结果
        //REQUEST_CODE_SCAN:请求码，也是回调中判断数据的来源，唯一值
        startActivityForResult(intentcapture,REQUEST_CODE_SCAN);
    }
