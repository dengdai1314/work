 ...
 * @date 2020/5/2017:55
 * @description 可爱 & 小资风格的加载等待自定义View https://www.jianshu.com/p/67b69fc8b63b
 */
public class Kawaii_LoadingView extends View {
    ...
    public Kawaii_LoadingView(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
...
//*****************************************************绘制方块********************************************************************************************************
    ...
    private void initAttrs(Context context,AttributeSet attrs){
        ...
        int defaultColor = context.getResources().getColor(R.color.colorAccent);//设置背景颜色
    ...
    private void initBlocks(int initPosition){
        ...
        mMoveBlock.rectF = new RectF();//设定方块
    ...
    private void relate_OuterBlock(fixedBlock[] fixedBlocks,boolean isClockwise){
        int lineCount = (int) Math.sqrt(fixedBlocks.length);//如果使用默认值3，那么fixedBlocks.lengt为9，Math.sqrt为3（开平方根）
    ...
    private void fixedBlockPosition(fixedBlock[] fixedBlocks,int cx,int cy,float dividerWidth,float halfSquareWidth){
        ...
        for(int i=0; i<lineCount; i++){//行
            for(int j=0; j<lineCount; j++){//列
                if(i == 0){
                    if(j == 0){
                        fixedBlocks[0].rectF.set(firstRectLeft,firstRectTop,firstRectLeft + squareWidth,firstRectTop + squareWidth);
                    }else {
     ...
     * 关注2：设置移动方块的位置
     */
    private void MoveBlockPosition(fixedBlock[] fixedBlocks, MoveBlock moveBlock, int initPosition, boolean isClockwise) {
...
//***************************************设置动画*******************************************************************************************************
    ...
    private ValueAnimator createTranslateValueAnimator(fixedBlock currEmptyfixedBlock, fixedBlock moveBlock) {
        ...
        if(isNextRollLeftOrRight(currEmptyfixedBlock, moveBlock)){
            ...
            //情况1：顺时针且在第一行 / 逆时针且在最后一行时，移动方向向右移动
            if(isClock_Wise && currEmptyfixedBlock.index > moveBlock.index || !isClock_Wise && currEmptyfixedBlock.index > moveBlock.index){
                ...
                //情况2：顺时针且在最后一行/逆时针且在第一行，移动方向向左移动
            ...
            //情况4：顺时针且在最右列/逆时针且在最左列，移动方向向下移动
            }else if (isClock_Wise && currEmptyfixedBlock.index > moveBlock.index
                || !isClock_Wise && currEmptyfixedBlock.index > moveBlock.index){
                startAnimValue = moveBlock.rectF.top;
                endAnimValue = moveBlock.rectF.top + blockInterval;
